...
export default function TodoItem({ todo, onToggle, onDelete, onRename }:{ 
  ...
  async function setCompleted(next: boolean){
    if (toggling) return;
    setToggling(true);
    try { await onToggle(todo.id, next); }
    ...
    <div className="card p-3 flex items-start gap-3 animate-fadeInUp">
      ...
      <div className="flex gap-2">
        {editing ? (
          ...
          <>
            {todo.completed ? (
              <button className="btn-ghost" disabled={toggling} onClick={()=>setCompleted(false)}>Mark Incomplete</button>
            ) : (
              <button className="btn-primary" disabled={toggling} onClick={()=>setCompleted(true)}>Mark Completed</button>
            )}
